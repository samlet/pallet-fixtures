...
decl_storage! {
	trait Store for Module<T: Trait> as CookieMap {
		CookieMap get(fn simple_map): map hasher(blake2_128_concat) T::AccountId => u32;
	}
}
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn get_single_entry(origin, account: T::AccountId) -> DispatchResult {
			...
			ensure!(<CookieMap<T>>::contains_key(&account), Error::<T>::NoValueStored);
			let entry = <CookieMap<T>>::get(account);
		...
		fn take_single_entry(origin) -> DispatchResult {
			...
			ensure!(<CookieMap<T>>::contains_key(&user), Error::<T>::NoValueStored);
			let entry = <CookieMap<T>>::take(&user);
